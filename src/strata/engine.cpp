...
Engine::Engine(const std::string& data_dir, size_t flush_threshold)
  ...
  for (const auto& name : ListFilesWithSuffix(data_dir_ + "/L0", ".blk")) {
    uint64_t id = std::strtoull(name.c_str(), nullptr, 10);
    if (id >= next_l0_id_) next_l0_id_ = id + 1;
  }
...
void Engine::Flush() {
  ...
  // Data just written to L0 is fsync'd and durable, so the WAL covering it
  // is no longer needed. Known simplification: a crash between the L0
  // fsync above and the WAL reset below leaves those points in both the
  // WAL and the new L0 block; next startup replays and re-flushes them
  // into a second block. Harmless today since nothing reads L0 data back
  // for real yet — worth revisiting once compaction/queries do (Phase 3+).
  wal_.reset();
