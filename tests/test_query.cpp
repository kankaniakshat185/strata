...
void TestBlockLevelPruning() {
  ...
  // L1 block C: single bucket at ts=20000.
  {
    ...
    strata::WriteL1Block(data_dir + "/L1/00000001.blk", {r});
  }
  // L1 block D: single bucket at ts=30000.
  {
    ...
    strata::WriteL1Block(data_dir + "/L1/00000002.blk", {r});
  }
  ...
  // Query 2: spans block B (L0) and block C (L1) -- the "stitch" case.
  {
    ...
    assert(r.series[0].rollup_buckets.size() == 1);
    assert(r.series[0].rollup_buckets[0].bucket_start == 20000);
  ...
  // Query 3: covers everything -- all 4 blocks scanned, none skipped.
  {
    auto r = engine.Query({"host=h1"}, 0, 40000);
    assert(r.series[0].raw_points.size() == 2);
    assert(r.series[0].rollup_buckets.size() == 2);
  ...
  // Query 4: a filter matching no series -- per STRATA_DESIGN.md's read
  // path ("the inverted index resolves the filter to candidate series
  // before any time-series data is touched"), zero matching series means
  // zero blocks should even be looked at.
  {
  ...
  // with no block scanned at all -- range doesn't overlap any of the 4
  // on-disk blocks.
  {
