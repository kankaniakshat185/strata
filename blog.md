# Strata: a Time-Series Database From Scratch in C++

you can't fully trust a bit-level compressor you wrote yourself. the encoder and the decoder came out of the same heads, working from the same mental model — so if that mental model has a bug in it, the encoder and the decoder can agree with each other perfectly and both be wrong about reality at the same time. there's no `gorilla-cli` sitting around to diff your output against. the only oracle you have is the code you're trying to verify. that problem, and what it actually takes to get past it, is most of what this post is about.

it's not the only thing this post is about, though. strata is a team project — built with [vaishnavi rai](https://github.com/VaishnaviRai287), start to finish. the write-ahead log, the memtable, the compression codec, compaction (now a four-level cascade), the crash-safety mechanism, two independent index structures, and the query router — all of it. we didn't build this by splitting it into two silos and never touching each other's code. we built the core together, kept extending it, and kept re-checking our own claims as new work got added, the same standard applied the whole way through regardless of which of us happened to be at the keyboard for a given line.

this post walks through it roughly in the order it actually got built: the durability and compression core first, then the parts that came later — a compaction cascade that goes four levels deep instead of one, a second index built specifically to find where the first one loses, and a crash-safety audit that caught a real gap in our own testing months after the code that gap was in had already shipped and already looked done.
...
writers ──> wal (fsync'd) ──> memtable ──> flush ──> l0 (gorilla-compressed, raw resolution)
              ...
              b+ tree: a second index, built to answer the one question the inverted index can't
...
the mechanism is almost insultingly simple to describe and genuinely fiddly to get exactly right: append every incoming point to a log file, and don't tell the caller the write succeeded until the operating system confirms those bytes are physically on disk — not a page cache that hasn't actually flushed to the device yet. a normal `write()` can return successfully while the data is still sitting in ram, waiting for the kernel to push it down later, purely because that's faster. that's the right tradeoff almost everywhere and the wrong one the instant a write needs to be provably durable.

`fsync()` is the call that closes that gap: it blocks until the kernel has actually pushed those specific bytes to the storage device, not just queued them. it's slow — tens of microseconds to a few milliseconds depending on the device — and strata pays that cost on literally every single write, on purpose, because durability that only applies "most of the time" isn't durability.
...
**the honest cost:** fsync-per-write is the reason strata's write throughput has a hard ceiling — measured flat around 30,000-33,000 points/second regardless of how many threads are writing at once (1, 2, 4, 8, 16, 32 — practically identical), which is exactly what you'd expect when every write serializes through one lock and then blocks on physical disk confirmation. the standard fix for this — batching multiple writers' records into one shared fsync, "group commit" — is a well-understood, well-scoped next step deliberately not built here. it's a real architectural change (writers need to coordinate on a batch boundary instead of each fsyncing independently), not a small patch, and it wasn't worth the time against a project that already proves durability-first design end to end. that ceiling is honest, not an oversight.
...
storing every point as a raw 16-byte pair is correct and wasteful — most real metrics barely move from one sample to the next. this piece i built on my own: **gorilla encoding** — not a paraphrase of the idea, an actual implementation of [facebook's 2015 vldb paper](https://www.vldb.org/pvldb/vol8/p1816-teller.pdf) on their in-memory time-series database, worked out by hand from the paper itself rather than ported from someone else's code. two tricks, one for timestamps and one for values, both exploiting the same underlying idea — encode the *difference* from the last point, and spend almost no bits when that difference is small or exactly repeats. the paper describes the shape of both tricks — the dod buckets, the xor windowing — but leaves real bit-packing choices open (the reuse trick in `EncodeDodField` below isn't spelled out anywhere in it), which is exactly why nothing existed to check the output against except the paper's description and a hand-worked trace.

before either trick makes sense, it's worth being exact about what a `double` actually looks like as bits, because the value-compression trick only makes sense once you can see it. ieee-754 double-precision floating point packs a number into 64 bits: 1 sign bit, 11 exponent bits (the number's scale), 52 mantissa bits (the significant digits). two numbers that are numerically close almost always share the same scale and most of the same leading digits — exactly why their bit patterns overlap so heavily, and exactly why xor works as a compression trick here. `65.0` is `0x4050400000000000` in that layout. `65.125` — barely different numerically — is `0x4050480000000000`. xor those two values together — a bit-by-bit comparison outputting 1 only where bits differ — and almost everything cancels: the sign bit matches, the exponent matches, most of the mantissa matches, and you're left with a handful of bits in the middle that actually changed. that's the entire insight the value codec is built on.
...
the other half of building trust was working the encoding out by hand before trusting the code to do it — the delta-of-delta table and the xor window table above aren't illustrative filler, they're a real trace, worked digit by digit against what the actual code emits, specifically so a mismatch between "what this should encode to" and "what it actually encodes to" would show up as a wrong number in a table sitting right there, not as a mysterious failure three files away pointing vaguely at "something in the codec." getting the leading-zero/trailing-zero window-reuse logic right — deciding when to reuse the previous xor's window versus paying to describe a new one — only became something worth trusting once it had survived both checks: a hand-traced example first, and a bit-exact round trip on real data second. a codec that's merely self-consistent (round-trips against itself) and a codec that merely matches a mental model on one hand-picked example are both individually dangerous. neither one alone would have caught everything. together, they're the reason "4.28x smaller" can go in this post as a measured fact instead of a number someone hoped was right.
...
the actual engineering discipline again wasn't the mechanism — it was proving it, not assuming it. five exact points in the code were named where a crash is genuinely dangerous — the moment right after the l0 block is fsynced but before the manifest knows about it; the moment right after the manifest is updated but before the wal is cleared; the same two moments on the compaction side — and each one got a real self-inflicted `SIGKILL` wired into it:
...
`SIGKILL` is the one signal a process can't intercept or handle — the kernel terminates it immediately, no cleanup code gets to run, no chance to close a file or finish a write in progress. that's exactly the property needed here: a real crash doesn't ask nicely either, and a test that let the process clean up gracefully first wouldn't actually be testing what a real crash does.

the reason this needs to be a self-inflicted kill rather than an external process racing a timer against the target: the crash windows worth testing are a few statements wide, sub-millisecond. an external `kill -9` has no way to reliably land inside a window that narrow — it's a coin flip whether the timing lines up at all. instrumenting the exact point and having the process kill itself on cue, keyed by an environment variable, is the standard technique for this (rocksdb calls it syncpoints; tikv and etcd call theirs fail-points), and it's the only way to get a deterministic, repeatable test out of a race condition that would otherwise be nearly impossible to hit on purpose.
...
fresh numbers, against 100 days of data with the last two days left "hot" in l0 and everything older compacted into l1 blocks:
...
this reproduces exactly — not approximately, exactly — the same 1/99, 3/97, 90/10 split first measured when the router was built, which makes sense: it's a deterministic count, not a timing measurement, so it can't drift the way latency numbers do. a 10-minute query against months of history touches one block. one honest nuance worth being precise about, because it's easy to overclaim: the router still opens every *candidate* block's header to check the overlap, even the ones it ends up skipping — the "skip" is cheap (one small file read, not a full decode), but it isn't free. what scales with total history isn't decode cost, it's a small, fixed per-block header check, and that distinction matters if you're reasoning precisely about where the cost actually goes instead of just citing the headline "1 block touched."

---

# extending it

that's the foundation: durability, compression, a downsampling pass, crash safety proven at five exact points, an index, and a router. everything from here down came later — not a patch on top of the original, a genuine extension of the same mechanisms, held to the same standard of "prove it, don't assert it" throughout.

## extending compaction from one hop to a cascade

single-level compaction turns raw l0 points into l1 hourly rollups and stops there. that's fine for weeks of retention. it stops being fine once you're keeping years of data — hourly buckets for three years is still a lot of rows. the fix was to make compaction recursive: l1's hourly buckets age into l2 daily buckets, and l2's daily buckets age into l3 monthly buckets, the same merge logic reapplied one level up each time.

the interesting part is that merging *summaries* needs different math than merging *raw points*, which needed a function that handles it explicitly rather than reusing the original `Downsample`:
...
it got left as-is rather than "fixed" — the actual fix isn't a bug patch, it's a real scope change: storing a proper streaming quantile sketch (a t-digest, an hdr histogram) per bucket instead of a single number, which is a legitimately different, bigger project. the point was to measure exactly how wrong the current shortcut gets and say so precisely, rather than let someone find out the hard way against real data later.

making a four-level cascade required generalizing structure that had only ever handled one rollup level. the manifest went from a flat `l1_blocks` list to an indexable array:
...
the manifest's text parser got tightened at the same time, in a direction that matters more than it looks: an earlier version of that parser would have silently dropped an unrecognized level token instead of failing loudly.
...
a pointer-to-member is a stored reference to *which field* to access later, not to any specific object — `&SeriesQueryResult::rollup_buckets_l1` points at the general concept "the l1-buckets field," and combining it with a real object later (`it->second->*out_field`) resolves it to that specific field on that instance. that's what lets one function body serve l1, l2, and l3 scanning at once — only the field pointer changes between calls, not the logic around it.
...
(`std::unique_ptr` auto-deletes the object it owns once nothing needs it anymore — c++'s explicit opt-in version of what garbage collection does invisibly in python or javascript.)
...
the honest expectation going in was that the hash map wins plain lookups and intersections easily, and it does, at every scale tested — a b+ tree pays real cost walking down through internal nodes that a hash map's single computed bucket index just skips entirely. the prefix scan is the real comparison, the one the hash map has no shortcut for at all, and the tree wins there decisively — the gap widens with scale too: about **2.1x** faster at 1,000 entries, up to roughly **3.6x** faster at 1,000,000. one nuance worth reporting honestly rather than smoothing over: order doesn't win monotonically on *every* axis at *every* scale — at 1,000,000 entries specifically, order 32's prefix-scan time (26.2ms) edges out order 128's (26.7ms) by a hair, close enough to be within normal run-to-run noise rather than a real trend. what order consistently and clearly buys you, at every scale, is memory — order 128 uses roughly 23-24% less than a bare hash map at a million entries, and clearly less than the narrower tree orders too. wider nodes mean fewer total nodes and less per-node pointer/vector overhead, and that part of the story holds cleanly; the prefix-scan-vs-order relationship is close enough among the wider orders that a clean monotonic curve there would need more samples than one run gives.

it's worth restating plainly: this tree is **not wired into the live engine**. it's benchmark-only, in-memory, built specifically to answer one comparative question — exactly the same reasoning behind why the cardinality benchmark drives `InvertedIndex` directly instead of through a live, disk-backed `Engine`: isolating a data structure's own performance from fsync latency doesn't require it to be sitting in the real write/query path.

## catching a gap in our own "proven" claim

this is the part that matters more than either of the two features above, so it's worth giving it the space it deserves rather than mentioning it in passing.

step back from the tree for a moment, to the cascade — because this next part happened alongside it, not after it. when the two new crash points for the l1-to-l2 manifest swap got added — the same shape of instrumented `SIGKILL` used for the original five — the follow-up was to go back and audit that claim the same way the original codec and crash-safety work had been held to: not "does this look right," but "have we actually gone and checked." what turned up was that the two new crash points had been correctly wired into the real compaction code, in exactly the right place, but the test harness meant to trigger them had never actually been taught their names:
...
the underlying mechanism got verified as sound by driving it manually first, then the gap got closed the same way the rest of the crash-safety suite is built: two new named scenarios in the real harness, each checked against an exact expected recovery outcome, not just "it didn't crash." deterministic fault-injection coverage went from five points to seven, and all seven pass for real now, not by resembling an already-proven case closely enough to pattern-match against.

that matters more than the multi-level cascade or the b+ tree, because of what it's evidence of, not what it built. the codec-trust discipline described earlier — don't trust it because it round-trips, go check the actual bits — isn't worth much if it only ever gets applied to the one thing that seemed worth being paranoid about in the first week. it's only actually a discipline if it survives getting applied again, later, to work that had already shipped and already looked done, without anything external forcing a second look. that's exactly what happened here.

---

## the numbers, together

**compression**, naive 16 bytes/point baseline: 3.74 B/pt on regular-cadence data (**4.28x** smaller), 7.11 B/pt on smooth-drift data (**2.25x** smaller).

**cardinality scaling** — inverted index lookup latency, p50/p99, 1K → 1M unique series: 209ns/625ns → 541ns/1,500ns. roughly 2.5x latency growth across a thousand-fold increase in scale, index size growing linearly the entire time.

**query planning** — 100 days of data, most of it aged into rollups: a 10-minute query touches 1 block and correctly skips 99; a 3-day query spanning both resolutions touches 3 and skips 97; a 90-day query touches 90 and skips 10. reproduces exactly.

**write throughput** — flat at roughly 30,000-33,000 points/second across 1 to 32 concurrent writer threads, because every write durably fsyncs before it's acknowledged. this is meaningfully lower than a figure measured on a different machine at an earlier point in the project (46,000-49,000/sec) — the shape (flat regardless of concurrency) held perfectly, the absolute number moved by more than the usual run-to-run drift, which is itself an honest data point about how much fsync latency depends on the specific disk underneath a given machine, not something to paper over.

**crash recovery** — 7 deterministic fault-injection points, each checked against an exact expected recovery outcome, all passing on a fresh run. plus one realistic external-kill harness.

**rollup accuracy** — merging two levels of rollups on data built to demonstrate the worst case: 89.1% divergence between the merged p99 estimate and the true value. the single most important honesty-check number in this whole project.

**inverted index vs. b+ tree** — hash map wins point lookup and intersection at every scale tested (542ns vs. 3,375-4,875ns at 1M entries). the b+ tree wins prefix scans, and the margin widens with scale: 2.1x at 1,000 entries, 3.6x at 1,000,000. wider tree nodes save real memory (~23-24% less at order 128 vs. a bare hash map, at 1M entries).

**test suite** — 10 binaries, all passing on a fresh build. roughly 3,137 lines of engine code and 1,337 lines of tests total, up from an original ~2,400/~900 before the cascade and the tree landed.
...
**ugly, and the part worth focusing on if you only have time for one thing:** the p99 rollup approximation is the sharpest edge in this whole system, and it compounds — a single level of compaction loses precision in a way that was described but never quantified; two levels of compaction, measured directly, diverges by 89.1% on data built to expose exactly that failure mode. and separately, the crash-safety suite itself once claimed coverage it didn't actually have — two fault-injection points that were real in the code but unreachable by the harness meant to exercise them, caught only because someone went back and checked a "proven" claim a second time instead of trusting it because it had passed before.

that last one is the actual thesis of this post. a system that's supposed to be correct at the bit level or the crash-recovery level doesn't get to just look correct in a demo or a green checkmark — it has to survive someone checking, on purpose, the way they'd check if they didn't already trust it. and it has to survive that twice: once when it's written, and again later, after it already looks finished. we did both. the number you haven't double-checked yet is the one worth double-checking first.

---

*written by akshat kankani and vaishnavi rai. source on [github](https://github.com/kankaniakshat185/strata).*
